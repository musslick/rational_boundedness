function fitVal = Manuscript_Simulation_3_TaskSwitchingUnivalent_fun(inputArg)
...
rep = inputArgDouble;
    ...
    %% NETWORK TRAINING
        ...
        for tauIdx = 1:length(tau_range)
            ...
            univalentStimuli = 0;
            congruent = 0;
            [repeatAccuarcy, switchAccuarcy, repeatRT, switchRT] = Part1_Sim3_TaskSwitching_Analysis(taskNet, input, train, tasksIdxSgl, taskA, taskB, congruent, tauNet, tauTask, nStimuli, verbose, univalentStimuli);
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).ER_bivalent_inc_rep = 1-repeatAccuarcy;
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).ER_bivalent_inc_switch = 1-switchAccuarcy;
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).ER_bivalent_inc_switchCost = repeatAccuarcy-switchAccuarcy;
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).RT_bivalent_inc_rep = repeatRT;
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).RT_bivalent_inc_switch = switchRT;
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).RT_bivalent_inc_switchCost = switchRT-repeatRT;
            
            % bivalent congruent
            univalentStimuli = 0;
            congruent = 1;
            [repeatAccuarcy, switchAccuarcy, repeatRT, switchRT] = Part1_Sim3_TaskSwitching_Analysis(taskNet, input, train, tasksIdxSgl, taskA, taskB, congruent, tauNet, tauTask, nStimuli, verbose, univalentStimuli);
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).ER_bivalent_con_rep = 1-repeatAccuarcy;
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).ER_bivalent_con_switch = 1-switchAccuarcy;
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).ER_bivalent_con_switchCost = repeatAccuarcy-switchAccuarcy;
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).RT_bivalent_con_rep = repeatRT;
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).RT_bivalent_con_switch = switchRT;
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).RT_bivalent_con_switchCost = switchRT-repeatRT;
            
            % univalent
            univalentStimuli = 1;
            congruent = [];
            [repeatAccuarcy, switchAccuarcy, repeatRT, switchRT] = Part1_Sim3_TaskSwitching_Analysis(taskNet, input, train, tasksIdxSgl, taskA, taskB, congruent, tauNet, tauTask, nStimuli, verbose, univalentStimuli);
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).ER_univalent_rep = 1-repeatAccuarcy;
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).ER_univalent_switch = 1-switchAccuarcy;
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).ER_univalent_switchCost = repeatAccuarcy-switchAccuarcy;
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).RT_univalent_rep = repeatRT;
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).RT_univalent_switch = switchRT;
            batch_log(numHiddenIdx, rep).structDependence(tauIdx).RT_univalent_switchCost = switchRT-repeatRT;
            ...
            congruent = 0;
            [repeatAccuarcy, switchAccuarcy, repeatRT, switchRT] = Part1_Sim3_TaskSwitching_Analysis(taskNet, input, train, tasksIdxSgl, taskA, taskB, congruent, tauNet, tauTask, nStimuli, verbose, univalentStimuli);
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).ER_bivalent_inc_rep = 1-repeatAccuarcy;
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).ER_bivalent_inc_switch = 1-switchAccuarcy;
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).ER_bivalent_inc_switchCost = repeatAccuarcy-switchAccuarcy;
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).RT_bivalent_inc_rep = repeatRT;
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).RT_bivalent_inc_switch = switchRT;
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).RT_bivalent_inc_switchCost = switchRT-repeatRT;
            
            % bivalent congruent
            univalentStimuli = 0;
            congruent = 1;
            [repeatAccuarcy, switchAccuarcy, repeatRT, switchRT] = Part1_Sim3_TaskSwitching_Analysis(taskNet, input, train, tasksIdxSgl, taskA, taskB, congruent, tauNet, tauTask, nStimuli, verbose, univalentStimuli);
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).ER_bivalent_con_rep = 1-repeatAccuarcy;
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).ER_bivalent_con_switch = 1-switchAccuarcy;
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).ER_bivalent_con_switchCost = repeatAccuarcy-switchAccuarcy;
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).RT_bivalent_con_rep = repeatRT;
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).RT_bivalent_con_switch = switchRT;
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).RT_bivalent_con_switchCost = switchRT-repeatRT;
            
            % univalent
            univalentStimuli = 1;
            congruent = [];
            [repeatAccuarcy, switchAccuarcy, repeatRT, switchRT] = Part1_Sim3_TaskSwitching_Analysis(taskNet, input, train, tasksIdxSgl, taskA, taskB, congruent, tauNet, tauTask, nStimuli, verbose, univalentStimuli);
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).ER_univalent_rep = 1-repeatAccuarcy;
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).ER_univalent_switch = 1-switchAccuarcy;
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).ER_univalent_switchCost = repeatAccuarcy-switchAccuarcy;
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).RT_univalent_rep = repeatRT;
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).RT_univalent_switch = switchRT;
            batch_log(numHiddenIdx, rep).funcDependence(tauIdx).RT_univalent_switchCost = switchRT-repeatRT;
            ...
            congruent = 0;
            [repeatAccuarcy, switchAccuarcy, repeatRT, switchRT] = Part1_Sim3_TaskSwitching_Analysis(taskNet, input, train, tasksIdxSgl, taskA, taskB, congruent, tauNet, tauTask, nStimuli, verbose, univalentStimuli);
            batch_log(numHiddenIdx, rep).independence(tauIdx).ER_bivalent_inc_rep = 1-repeatAccuarcy;
            batch_log(numHiddenIdx, rep).independence(tauIdx).ER_bivalent_inc_switch = 1-switchAccuarcy;
            batch_log(numHiddenIdx, rep).independence(tauIdx).ER_bivalent_inc_switchCost = repeatAccuarcy-switchAccuarcy;
            batch_log(numHiddenIdx, rep).independence(tauIdx).RT_bivalent_inc_rep = repeatRT;
            batch_log(numHiddenIdx, rep).independence(tauIdx).RT_bivalent_inc_switch = switchRT;
            batch_log(numHiddenIdx, rep).independence(tauIdx).RT_bivalent_inc_switchCost = switchRT-repeatRT;
            
            % bivalent congruent
            univalentStimuli = 0;
            congruent = 1;
            [repeatAccuarcy, switchAccuarcy, repeatRT, switchRT] = Part1_Sim3_TaskSwitching_Analysis(taskNet, input, train, tasksIdxSgl, taskA, taskB, congruent, tauNet, tauTask, nStimuli, verbose, univalentStimuli);
            batch_log(numHiddenIdx, rep).independence(tauIdx).ER_bivalent_con_rep = 1-repeatAccuarcy;
            batch_log(numHiddenIdx, rep).independence(tauIdx).ER_bivalent_con_switch = 1-switchAccuarcy;
            batch_log(numHiddenIdx, rep).independence(tauIdx).ER_bivalent_con_switchCost = repeatAccuarcy-switchAccuarcy;
            batch_log(numHiddenIdx, rep).independence(tauIdx).RT_bivalent_con_rep = repeatRT;
            batch_log(numHiddenIdx, rep).independence(tauIdx).RT_bivalent_con_switch = switchRT;
            batch_log(numHiddenIdx, rep).independence(tauIdx).RT_bivalent_con_switchCost = switchRT-repeatRT;
            
            % univalent
            univalentStimuli = 1;
            congruent = [];
            [repeatAccuarcy, switchAccuarcy, repeatRT, switchRT] = Part1_Sim3_TaskSwitching_Analysis(taskNet, input, train, tasksIdxSgl, taskA, taskB, congruent, tauNet, tauTask, nStimuli, verbose, univalentStimuli);
            batch_log(numHiddenIdx, rep).independence(tauIdx).ER_univalent_rep = 1-repeatAccuarcy;
            batch_log(numHiddenIdx, rep).independence(tauIdx).ER_univalent_switch = 1-switchAccuarcy;
            batch_log(numHiddenIdx, rep).independence(tauIdx).ER_univalent_switchCost = repeatAccuarcy-switchAccuarcy;
            batch_log(numHiddenIdx, rep).independence(tauIdx).RT_univalent_rep = repeatRT;
            batch_log(numHiddenIdx, rep).independence(tauIdx).RT_univalent_switch = switchRT;
            batch_log(numHiddenIdx, rep).independence(tauIdx).RT_univalent_switchCost = switchRT-repeatRT;
...
if(~trainMultitasking)
    save(['logfiles/Part1/PsychReview_Part1_Sim3_TaskSwitchingUnivalent_' num2str(NPathways) 'P' num2str(NFeatures) 'F_' num2str(length(tasksToPerform)) 'tasks_' num2str(log_version) '_h' num2str(numHiddenUnits(1)) '_r' num2str(rep)]);
else
    save(['logfiles/Part1/PsychReview_Part1_Sim3_TaskSwitchingUnivalent_multi_' num2str(NPathways) 'P' num2str(NFeatures) 'F_' num2str(length(tasksToPerform)) 'tasks_' num2str(log_version) '_h' num2str(numHiddenUnits(1)) '_r' num2str(rep)]);
end
