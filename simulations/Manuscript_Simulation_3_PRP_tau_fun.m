function fitVal = Manuscript_Simulation_3_PRP_tau_fun(inputArg)
    ...
    %% NETWORK TRAINING
        ...
        if(~skipPRPAnalysis)
            ...
            for tau_idx = 1:length(tau_range)
                ...
                batch_log(numHiddenIdx, rep).funcDependence(tau_idx).task1RT_all_log = task1RT_all_log;
                ...
                batch_log(numHiddenIdx, rep).funcDependence(tau_idx).task2RT_all_log = task2RT_all_log;
                ...
                for SOA_idx = 1:length(SOA_range)
                    ...
                    task1RT_inc_log(SOA_idx) = mean(task1RT_all(incongruentIdx));
                    task1RT_all_inc_log{SOA_idx} = task1RT_all;
                    task2RT_inc_log(SOA_idx) = mean(task2RT_all(incongruentIdx));
                    task2RT_all_inc_log{SOA_idx} = task2RT_all;
                    
                    task1RT_con_log(SOA_idx) = mean(task1RT_all(congruentIdx));
                    task1RT_all_con_log{SOA_idx} = task1RT_all;
                    task2RT_con_log(SOA_idx) = mean(task2RT_all(congruentIdx));
                    task2RT_all_con_log{SOA_idx} = task2RT_all;
                end

                batch_log(numHiddenIdx, rep).independence(tau_idx).task1RT_log = task1RT_log;
                batch_log(numHiddenIdx, rep).independence(tau_idx).task1RT_all_log = task1RT_all_log;
                ...
                batch_log(numHiddenIdx, rep).independence(tau_idx).task2RT_all_log = task2RT_all_log;
                ...
                batch_log(numHiddenIdx, rep).independence_inc(tau_idx).task1RT_log = task1RT_inc_log;
                batch_log(numHiddenIdx, rep).independence_inc(tau_idx).task1RT_all_log = task1RT_all_inc_log;
                batch_log(numHiddenIdx, rep).independence_inc(tau_idx).task2RT_log = task2RT_inc_log;
                batch_log(numHiddenIdx, rep).independence_inc(tau_idx).task2RT_all_log = task2RT_all_inc_log;
                
                batch_log(numHiddenIdx, rep).independence_con(tau_idx).task1RT_log = task1RT_con_log;
                batch_log(numHiddenIdx, rep).independence_con(tau_idx).task1RT_all_log = task1RT_all_con_log;
                batch_log(numHiddenIdx, rep).independence_con(tau_idx).task2RT_log = task2RT_con_log;
                batch_log(numHiddenIdx, rep).independence_con(tau_idx).task2RT_all_log = task2RT_all_con_log;
...
if(~trainMultitasking)
    ...
    save(['logfiles/Part1/PsychReview_Part1_Sim4_PRP_tau_multi_' num2str(NPathways) 'P' num2str(NFeatures) 'F_' num2str(length(tasksToPerform)) 'tasks_' num2str(log_version) '_h' num2str(numHiddenUnits(1)) '_r' num2str(rep)]);
end
