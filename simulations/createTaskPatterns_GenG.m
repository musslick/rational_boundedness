...
% flag whether to output multitasking patterns
generateMultitaskingPatterns = 1;
if(~isempty(varargin))
    if(length(varargin) >= 2)
        generateMultitaskingPatterns = varargin{2};
    end
end

% flag whether to zero out dimensions
zeroDimensions = 0;
if(~isempty(varargin))
    if(length(varargin) >= 3)
        zeroDimensions = varargin{3};
    end
end

if(zeroDimensions)
   NFeatures = NFeatures + 1; % create additional feature for 0 dimension 
end
...
if(zeroDimensions)
    zeroFeatureIdx_list = nan(1,NPathways);
    for i = 1:NPathways
       zeroFeatureIdx_list(i) = (i - 1) * NFeatures + NFeatures;
    end
    inputSgl(:, zeroFeatureIdx_list) = [];
    trainSgl(:, zeroFeatureIdx_list) = []; 
    NFeatures = NFeatures - 1;
end

%% create multitasking patterns
NUM_SGL_TASKS = length(relevantTasks);
taskIdx = repmat(1:NUM_SGL_TASKS, [nStims,1]); taskIdx = taskIdx(:);
multiCap{1}.taskIdx = taskIdx;
multiCap{1}.tasksIdxSgl = tasksIdxSgl;

multiCap{1}.input = inputSgl;
multiCap{1}.tasks = tasksSgl;
multiCap{1}.train = trainSgl;
multiCap{1}.taskCombs = transpose(relevantTasks);
if(generateMultitaskingPatterns)
    ...
    for Nactive = 2:NPathways
        ...
        for currTaskComb = 1:size(taskCombs,2)
            ...
            if( mean(mean((relevantTaskM & currTasksM) == currTasksM)) == 1)
                ...
                currentTaskCombination = find(taskPattern == 1);
                currentTaskCombinations = [currentTaskCombinations; currentTaskCombination];
        ...
        multiCap{Nactive}.taskCombs = currentTaskCombinations;
        ...
        if(zeroDimensions)
            ...
            multiCap{Nactive}.input(:, zeroFeatureIdx_list) = [];
            multiCap{Nactive}.train(:, zeroFeatureIdx_list) = [];
        end
