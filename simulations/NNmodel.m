    ...
    methods
        ...
        function this = NNmodel(varargin)
            ...
            if(isa(varargin{1},'NNmodel'))
                ...
                this.isFixedInputHiddenW = copy.isFixedInputHiddenW;
                this.isFixedTaskHiddenW = copy.isFixedTaskHiddenW;
                this.isFixedTaskOutputW = copy.isFixedTaskOutputW;
                this.isFixedHiddenOutputW = copy.isFixedHiddenOutputW;
                return;
        ...
        function [outData, hiddenData, MSE, hidden_net, output_net, ceError, classError, classDimError, hidden_net_input, hidden_net_task, hidden_net_bias, output_net_hidden, output_net_task, output_net_bias] = runSet(this, varargin)
            ...
            ceError = -1*ones(1,Ndatasets);
            classError = -1*ones(1,Ndatasets);
            classDimError = -1*ones(1,Ndatasets);
            if(length(varargin)>=3)
                trainData = varargin{3};
        ...
        function [output_act, hidden_act, MSE, hidden_net, output_net] = runTrial(this, input, task, varargin)
            ...
            hidden_net_task = this.weights.W_TH * transpose(task);      % input from task layer (task cue)
            ...
            % calculate net input for output units
            output_net_task = this.weights.W_TO * transpose(task);      % input from task layer (task cue)
            output_net_hidden = this.weights.W_HO * this.hidden_act;            % input from hidden layer
       ...
       % be executed by appying the softmax function to the output patterns
        ...
        optimizeAcrossPatterns_default = 0;
        showWarning = 0;
        ...
        if(isfield(LCA_settings, 'ITI'))
            ITI = LCA_settings.ITI;
        else
            ITI = ITI_default;
            if(showWarning) 
                warning(['No ITI specified, will use default value of ' num2str(numSimulations) ' for reward rate calculation.']);
            end
        end
       ...
       if(isfield(LCA_settings,'optimizeAcrossPatterns'))
            optimizeAcrossPatterns = LCA_settings.optimizeAcrossPatterns;
        else
            optimizeAcrossPatterns = optimizeAcrossPatterns_default;
        end
        ...
        LCA_input = nan(numRespPatterns, NFeatures, size(output,3)); % matrix that contains each response dimension
        ...
        % LCA input dimensions
        LCA_input_dim = [size(LCA_input, 1) size(LCA_input,2)];
        
        % initialize each LCA
        x = zeros([LCA_input_dim numSimulations maxTimeSteps]); % net input
        f = zeros([LCA_input_dim numSimulations maxTimeSteps]); % activation

        % generate noise for all LCAs and timesteps
        noise = c.*randn([LCA_input_dim numSimulations maxTimeSteps]);

        if(size(LCA_input,3) == 1)
            % compute external input I_ext = p
            p = squeeze(LCA_input(:,:,1)) * W_ext;
        ...
        for t = 1:maxTimeSteps
            ...
            if(t==1)
                ...
                if(size(LCA_input,3) > 1)
                    % compute external input I_ext = p
                    t_input = min([size(LCA_input,3), t]);
                    p = squeeze(LCA_input(:,:,t_input)) * W_ext;
                    p = repmat(p, [1, 1, numSimulations]); 
                end
        ...
        RT_Simulations_correct = nan(numThresholds, numRespPatterns, numSimulations);
        RT_Simulations_incorrect = nan(numThresholds, numRespPatterns, numSimulations);
...
%         squeeze(f(3, :, 1, :))
        ...
        for patternIdx = 1:numRespPatterns
            ...
            correctFeature = find(LCA_train(patternIdx,:) == 1);
            ...
            for simIdx = 1:numSimulations
                ...
                for thresholdIdx = 1:numThresholds
                    ...
                    % separate correct vs. incorrect RTs
                    
                    if(winningFeature == correctFeature)
                        RT_Simulations_correct(thresholdIdx, patternIdx, simIdx) = RT_Simulations(thresholdIdx, patternIdx, simIdx);
                    else
                        RT_Simulations_incorrect(thresholdIdx, patternIdx, simIdx) = RT_Simulations(thresholdIdx, patternIdx, simIdx);
                    end
...
%            disp(['pattern ' num2str(patternIdx) '/' num2str(numRespPatterns)]);
        ...
        if(numThresholds > 1)
            ...
            RTs_correct = nanmean(RT_Simulations_correct, 3);
            RTs_incorrect = nanmean(RT_Simulations_incorrect, 3);
        else
            RTs(1, :) = mean(RT_Simulations, 3);
            RTs_correct(1,:) = nanmean(RT_Simulations_correct, 3);
            RTs_incorrect(1,:) = nanmean(RT_Simulations_incorrect, 3);
        end
        ...
        taskRT_correct = nan(numThresholds, numPatterns, size(tasks,2));
        taskRT_incorrect = nan(numThresholds, numPatterns, size(tasks,2));
        taskRT_correct_all = nan(numThresholds, numPatterns, size(tasks,2), numSimulations);
        taskRT_incorrect_all = nan(numThresholds, numPatterns, size(tasks,2), numSimulations);
        ...
        for threshIdx = 1:numThresholds
            ...
            for patternIdx = 1:size(output,1)
                ...
                for taskIdx = 1:length(currentTasks)
                    ...
                    taskRT_correct(threshIdx, patternIdx, currentTask) = RTs_correct(threshIdx, outputCounter);
                    taskRT_incorrect(threshIdx, patternIdx, currentTask) = RTs_incorrect(threshIdx, outputCounter);
                    taskRT_correct_all(threshIdx, patternIdx, currentTask,:) = RT_Simulations_correct(threshIdx, outputCounter,:);
                    taskRT_incorrect_all(threshIdx, patternIdx, currentTask,:) = RT_Simulations_incorrect(threshIdx, outputCounter,:);
        ...
        meanRT_correct = nanmean(taskRT_correct, 3);
        meanRT_incorrect = nanmean(taskRT_incorrect, 3);
         ...
         optTaskRT_correct= nan(size(taskRT,2), size(taskRT,3));
         optTaskRT_incorrect= nan(size(taskRT,2), size(taskRT,3));
         optAccuracy = nan(1, size(meanAccuracy, 2));
         optRT = nan(1, size(meanRT, 2));
         optRT_correct = nan(1, size(meanRT, 2));
         optRT_incorrect = nan(1, size(meanRT, 2));
         ...
         maxThresholdReachedPatternTaskPairs = [];
          ...
          % optimize RR for each task across all patterns
        if(optimizeAcrossPatterns)
            
                RR = taskAccuracy ./ (ITI + taskRT);

                optThreshIdx = nan(size(taskAccuracy, 2), size(taskAccuracy, 3));
                % identify relevant tasks
                usedTasks = 1:size(tasks,2);
                usedTasks(sum(tasks) == 0) = [];
                
                for taskIdx = 1:length(usedTasks)
                    
                    task = usedTasks(taskIdx);
                    taskRelevantPatterns = tasks(:,task)==1;
                    
                    RR_tasks = nanmean(RR(:, taskRelevantPatterns, task), 2);
                    optThresh = find(RR_tasks == max(RR_tasks));
                    
                    if(isempty(optThresh))
                        optThresh = 1;
                        warning(['Could not find optimal threshold for task ' num2str(taskIdx) '. Consider increasing number of maximum time steps.']);
                    end

                    if(responseThreshold(optThresh) == min(responseThreshold))
                        warning(['Picked minimum threshold for task ' num2str(taskIdx) ': '  num2str(responseThreshold(optThresh)) ' . Consider increasing range of tested thresholds.']);
                    ...
                    if(responseThreshold(optThresh) == max(responseThreshold))
                        warning(['Picked maximum threshold for task ' num2str(taskIdx) ': ' num2str(responseThreshold(optThresh)) ' . Consider increasing range of tested thresholds.']);
                        maxThresholdReachedPatternTaskPairs = [maxThresholdReachedPatternTaskPairs; patternIdx taskIdx];
                    end
                    
                    optThreshIdx(:, task) = optThresh(1);
                    optTaskAccuracy(:, task) = taskAccuracy(optThresh(1), :, task);
                    optTaskRT(:, task) = taskRT(optThresh(1), :, task);
                    optTaskRT_correct(:, task) = taskRT_correct(optThresh(1), :, task);
                    optTaskRT_incorrect(:, task) = taskRT_incorrect(optThresh(1), :, task);
                end

                optAccuracy = nanmean(optTaskAccuracy, 2);
                optRT = nanmean(optTaskRT, 2);
                optRT_correct = nanmean(optTaskRT_correct, 2);
                optRT_incorrect = nanmean(optTaskRT_incorrect, 2);
            
        else % optimize RR pattern by pattern
            if(optimizeTasksWithinPattern)
                ...
                for task = 1:length(relevantTasks)
                    ...
                    if(responseThreshold(optThresh) == min(responseThreshold) & min(responseThreshold) > 0)
                        warning(['Picked minimum threshold for task ' num2str(taskIdx) ' in pattern ' num2str(patternIdx) ': ' num2str(responseThreshold(optThresh)) ' . Consider increasing range of tested thresholds.']);
                        minThresholdReachedPatternTaskPairs = [minThresholdReachedPatternTaskPairs; patternIdx taskIdx];
                    end

                    if(responseThreshold(optThresh) == max(responseThreshold))
                        warning(['Picked maximum threshold for task ' num2str(taskIdx) ' in pattern ' num2str(patternIdx) ': ' num2str(responseThreshold(optThresh)) ' . Consider increasing range of tested thresholds.']);
                        maxThresholdReachedPatternTaskPairs = [maxThresholdReachedPatternTaskPairs; patternIdx taskIdx];
                    end
                    ...
                    optTaskRT_correct(patternIdx, taskIdx) = taskRT_correct(optThresh(1), patternIdx, taskIdx);
                    optTaskRT_incorrect(patternIdx, taskIdx) = taskRT_incorrect(optThresh(1), patternIdx, taskIdx);
                end
                ...
                for patternIdx = 1:size(meanAccuracy, 2)
                    ...
                    optTaskRT_correct(patternIdx, :) = taskRT_correct(optThresh(1), patternIdx, :);
                    optTaskRT_incorrect(patternIdx, :) = taskRT_incorrect(optThresh(1), patternIdx, :);

                    optAccuracy(patternIdx) = meanAccuracy(optThresh(1), patternIdx);
                    optRT(patternIdx) = meanRT(optThresh(1), patternIdx);
                    optRT_correct(patternIdx) = meanRT_correct(optThresh(1), patternIdx);
                    optRT_incorrect(patternIdx) = meanRT_incorrect(optThresh(1), patternIdx);
        ...
        [settings,ok]= checkSettings(settings);
        ...
        [LCAParams,ok]= checkLCAPars(LCAParams);
        ...
        for thresholdIdx = 1:numThresholds
            ...
            % run LCA
            rng('default')
            [distributions, ok] = LCADist(LCAParams,settings);
        ...
        % calculate switch dynamics between inputs based on delayed net input
        function [output_act_log, hidden_act_log, MSE, DDM_RTs, DDM_ERs, E, A] = switchTasks(this, tauNet, tauTask, iterations, inputA, inputB, tasksA, tasksB, varargin)
            ...
            % get first data
            [output_act_A, hidden_act_A, MSE_A, hidden_net_A, output_net_A, ~, ~, ~, ...
                hidden_net_A_input, hidden_net_A_task, hidden_net_A_bias, ...
                output_net_A_hidden, output_net_A_task, output_net_A_bias] = runSet(this, inputA, tasksA, trainA);

            output_net_log(:,1,:) = output_net_A;
            output_net_log_bias(:,1,:) = output_net_A_bias';
            output_net_log_task(:,1,:) = output_net_A_task';
            output_net_log_hidden(:,1,:) = output_net_A_hidden';
            output_act_log(:,1,:) = output_act_A;
            hidden_net_log(:,1,:) = hidden_net_A;
            hidden_net_log_bias(:,1,:) = hidden_net_A_bias';
            hidden_net_log_task(:,1,:) = hidden_net_A_task';
            hidden_net_log_input(:,1,:) = hidden_net_A_input';
            ...
            for set = 1:Nsets
                ...
                for i = 2:iterations
                    ...
                    hidden_net_log_bias(set,i,:) = tauNet*hidden_net_bias + (1-tauNet) * squeeze(hidden_net_log_bias(set,i-1,:)); % integrate input from previous time step and current time step
                    hidden_net_log_task(set,i,:) = tauTask*hidden_net_task + (1-tauTask) * squeeze(hidden_net_log_task(set,i-1,:)); % integrate input from previous time step and current time step
                    hidden_net_log_input(set,i,:) = tauNet*hidden_net_input + (1-tauNet) * squeeze(hidden_net_log_input(set,i-1,:)); % integrate input from previous time step and current time step
                    
                    hidden_net_log(set,i,:) = hidden_net_log_bias(set,i,:) + hidden_net_log_task(set,i,:) + hidden_net_log_input(set,i,:); % integrate input from previous time step and current time step
                    ...
                    output_net_log_bias(set,i,:) = tauNet*output_net_bias + (1-tauNet) * squeeze(output_net_log_bias(set,i-1,:)); % integrate input from previous time step and current time step
                    output_net_log_task(set,i,:) = tauTask*output_net_task + (1-tauTask) * squeeze(output_net_log_task(set,i-1,:)); % integrate input from previous time step and current time step
                    output_net_log_hidden(set,i,:) = tauNet*output_net_hidden + (1-tauNet) * squeeze(output_net_log_hidden(set,i-1,:)); % integrate input from previous time step and current time step

                    output_net_log(set,i,:) = output_net_log_bias(set,i,:) + output_net_log_task(set,i,:) + output_net_log_hidden(set,i,:);% integrate input from previous time step and current time step
...
%                 end
        ...
        % integrate network activity over time
        function [output_act_log, hidden_act_log] = integrate(this, tauNet, tauTask, input_series, tasks_series)
            
           iterations = size(input_series,3);
            
           % prepare log data
           Nsets = size(input_series,1);
           output_net_log = nan(Nsets, iterations, this.Noutput);
           output_act_log = nan(Nsets, iterations, this.Noutput);
           hidden_net_log = nan(Nsets, iterations, this.Nhidden);
           hidden_act_log = nan(Nsets, iterations, this.Nhidden);
           MSE = nan(Nsets, iterations);
            ...
            % get first data
            inputA = squeeze(input_series(:,:,1));
            tasksA = squeeze(tasks_series(:,:,1));
            [output_act_A, hidden_act_A, ~, hidden_net_A, output_net_A, ~, ~, ~, ...
                hidden_net_A_input, hidden_net_A_task, hidden_net_A_bias, ...
                output_net_A_hidden, output_net_A_task, output_net_A_bias] = runSet(this, inputA, tasksA);
            ...
            for set = 1:Nsets
                ...
                for i = 2:iterations
                    ...
                    input = squeeze(input_series(set,:,i));
                    task = squeeze(tasks_series(set,:,i));
        ...
        % input
        function [output_act_log, hidden_act_log, MSE, DDM_RTs, DDM_ERs, E, A] = switchTasksSimple(this, tau, iterations, inputA, inputB, tasksA, tasksB, varargin)
