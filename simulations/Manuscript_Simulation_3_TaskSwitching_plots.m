...
fig = figure(1);
set(fig, 'Position', [100 400 900 200]);
x = [1, 2];
ylimit = [0 0.5];
% xlimit = [0.5 2.5];
lineWidth = 3;
...
errorbar(x, y_switch_mean*scale, y_switch_sem*scale, '-k', 'LineWidth', lineWidth); hold on;
errorbar(x, y_rep_mean*scale, y_rep_sem*scale, '--k', 'LineWidth', lineWidth); hold off;
ylim(ylimit);
xlim([0.8 2.2]);
