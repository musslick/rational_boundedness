function [CDFs] = getTownsendCDF(taskNet, NPathways, type0Idx, multiCap)
  ...
  CDFs = {};
    ...
    for cap = 2:2
        ...
        if(~isempty(allTaskCombs))
            ...
            if(cap == 1)
                maxIter = size(allTaskCombs, 1);
            elseif(cap == 2)
                maxIter = length(type0Idx);
            end
            for combIdx = 1:maxIter

                % find corresponding test patterns
                if(cap == 1)
                    patternIdx = multiCap{cap}.taskIdx == combIdx;
                elseif(cap == 2)
                    patternIdx = multiCap{cap}.taskIdx == type0Idx(combIdx);
                end
                ...
                if(cap == 2)
                    taskA = allTaskCombs(type0Idx(combIdx),1);
                    taskB = allTaskCombs(type0Idx(combIdx),2);
                    ...
                    [A_B, A_B_1, min_A_B, x, C]  = computeTownsendWengerCDF(taskRT_correct_AB_all, taskRT_correct_A_all, taskRT_correct_B_all, taskA, taskB, optThreshIdx_A, optThreshIdx_B, optThreshIdx_AB, thresholdIdx); % thresholdIdx
                ...
                disp(['cap: ' num2str(cap) ', comb ' num2str(combIdx) '/' num2str(maxIter)]);
            end
