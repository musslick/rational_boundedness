...
datafilePrefix = 'PsychReview_Part1_Sim1_sharedRepresentation_h100_N_5';
datafileSuffix = '_4.mat';
correlation_threshold = 0.5;
optimalPerformanceThreshold = 0.90;
graphSpecsFolder_selected = 'N_5_z_1';
samplingFolder_selected = 'ER'; % CM ER regular
graphsPerSpecFolder = 40;
maxN = 8;

logFolder = 'logfiles/Part1/';
...
for i =1:length(files)
    ...
    if(~isempty(strfind(files(i).name, datafilePrefix)) && ~isempty(strfind(files(i).name, datafileSuffix)))
...
maximumPerformanceCurveLog.CorrectResponseAct = nan(numGraphs, numRepetitions, maxN);
...
minimumPerformanceCurveLog.CorrectResponseAct = nan(numGraphs, numRepetitions, maxN);
...
maximumPerformanceCurveLog_spec.CorrectResponseAct = nan(graphsPerSpecFolder, numRepetitions, maxN);
...
minimumPerformanceCurveLog_spec.CorrectResponseAct = nan(graphsPerSpecFolder, numRepetitions, maxN);
...
for A_ID = 1:numGraphs
    ...
    fieldtmp = graph_log(A_ID).maximumPerformanceCurveLog;
    if(isfield(fieldtmp, 'CorrectResponseAct'))
        maximumPerformanceCurveLog.CorrectResponseAct(A_ID,:,1:maxCardinaility) = graph_log(A_ID).maximumPerformanceCurveLog.CorrectResponseAct;
    end
    ...
    fieldtmp = graph_log(A_ID).minimumPerformanceCurveLog;
    if(isfield(fieldtmp, 'CorrectResponseAct'))
        minimumPerformanceCurveLog.CorrectResponseAct(A_ID,:,1:maxCardinaility) = graph_log(A_ID).minimumPerformanceCurveLog.CorrectResponseAct;
    end
...
for A_ID = 1:numGraphs
    if(strcmp(graph_log(A_ID).graphSpecsFolder, graphSpecsFolder_selected) && strcmp(graph_log(A_ID).samplingFolder, samplingFolder_selected))
        ...
        fieldtmp = graph_log(A_ID).maximumPerformanceCurveLog;
        if(isfield(fieldtmp, 'CorrectResponseAct'))
            maximumPerformanceCurveLog_spec.CorrectResponseAct(spec_counter,:,1:maxCardinaility) = graph_log(A_ID).maximumPerformanceCurveLog.CorrectResponseAct;
        end
    
        minimumPerformanceCurveLog_spec.absoluteAccuracy(spec_counter,:,1:maxCardinaility) = graph_log(A_ID).minimumPerformanceCurveLog.absoluteAccuracy;
        minimumPerformanceCurveLog_spec.PCorrect(spec_counter,:,1:maxCardinaility) = graph_log(A_ID).minimumPerformanceCurveLog.PCorrect;
        minimumPerformanceCurveLog_spec.LCA(spec_counter,:,1:maxCardinaility) = graph_log(A_ID).minimumPerformanceCurveLog.LCA;
        fieldtmp = graph_log(A_ID).minimumPerformanceCurveLog;
        if(isfield(fieldtmp, 'CorrectResponseAct'))
            minimumPerformanceCurveLog_spec.CorrectResponseAct(spec_counter,:,1:maxCardinaility) = graph_log(A_ID).minimumPerformanceCurveLog.CorrectResponseAct;
        end
...
optimalPerformanceThreshold = 0.9; % threshold on performance to decide whether the multitasking attempt was successful or not (only necessary for effective PPC metric)
...
switch performanceMetric
    ...
    case 'CorrectResponseAct'
        ...
        switch performanceType
            case 'max'
                performanceData = maximumPerformanceCurveLog.CorrectResponseAct;
            case 'min'
                performanceData = minimumPerformanceCurveLog.CorrectResponseAct;
        end
...
fig1 = figure(1);
set(fig1, 'Position', [500 500 400 350]);
...
if(showSingleGraph == 0 | showSingleGraph ~= 0)
    ...
    legendLabels{end+1} = '';
    leg = legend(h, legendLabels,'Location','southwest');
...
if(strcmp(performanceMetric, 'LCA'))
    ylabelString = {'Multitasking Accuracy in %'};
    ylim([0 100]);
...
% parameters 
correlation_threshold = 0.5000;   % correlation threshold used to extract unweighted graph and compute MIS
optimalPerformanceThreshold = 0.90; % threshold on performance to decide whether the multitasking attempt was successful or not (only necessary for effective PPC metric)
cardinality_window = -3:3;  % plotted cardinality relative to predicted MIS
performanceMetric = 'absoluteAccuracy';         % this could be absoluteAccuracy, PCorrect, LCA
performanceType = 'max';                                % this could be min, max, effective
...
% draw figure
fig1 = figure(1);
set(fig1, 'Position', [100 100 1400 400]);
...
for N_idx = 1:length(plottedNetworkSizes)
    ...
    ylim([0 100]);
    xlim([0.5, length(cardinality_window) + 0.5]);
    title(['N = ' num2str(current_N)], 'FontSize', fontSize_title-1);
    ylabel(ylabelString, 'FontSize', fontSize_ylabel-2);
    xlabel('Task Set Size', 'FontSize', fontSize_ylabel-2);
