function [task1RT, task1Accuracy, task2RT, task2Accuracy, task1Iterations, task2Iterations, task1RT_all, task2RT_all] = Part1_Sim4_PRP_Analysis_V2(SOA, taskNet, input, train, tasksIdxSgl, taskA, taskB, congruent, tauNet, tauTask, nStimuli, varargin)
...
probedOnsets = SOA:1:(SOA+min(max(10, SOA*5), maxTimeSteps-SOA));
...
for onsetTaskB = probedOnsets
    ...
    disp(['Testing time step ' num2str(onsetTaskB) '/' num2str(onsetTaskB+length(probedOnsets))]);
    ...
    if(plot)
       ...
       subplot(1,4,4);
       imagesc(transpose(squeeze(trainA(patternIdx,:))));
    end
...
disp(['best onset index: ' num2str(bestRRIdx) '/' num2str(length(totalRewardRate))]);
