function fitVal = Manuscript_Simulation_3_PRP_BCE_tau_fun(inputArg)
...
rep = inputArgDouble;
    ...
    %% NETWORK TRAINING
        ...
        % 1) pre-train on location tasks
        MSE_log = zeros(1, iterations_Train);
        
        for iter =1:iterations_Train
            % sample primary training patterns
            samplesPerTask = samplesPerTask_train;
            [input_single, tasks_single, train_single] = createTaskPatterns(NPathways, NFeatures, samplesPerTask, sdScale, sameStimuliAcrossTasks, tasksPreTrain);
            
            % train network on samples
            taskNet.trainOnline(1, input_single, tasks_single, train_single);
        ...
        % 2) train on the two primary tasks 
        ...
        for iter =1:iterations_Train
            ...
            [input_single, tasks_single, train_single] = createTaskPatterns(NPathways, NFeatures, samplesPerTask, sdScale, sameStimuliAcrossTasks, taskPair_dependent);

            % remove location information
            input_single(:, ((NPathways-1)*NFeatures+1):((NPathways)*NFeatures)) = 0;
            ...
            if(trainMultitasking)
                ...
                % put it all together
                [input_locationTasks, tasks_locationTasks, train_locationTasks] = createTaskPatterns(NPathways, NFeatures, samplesPerTask, sdScale, sameStimuliAcrossTasks, tasksPreTrain);
                
                input = [input_single; input_locationTasks];
                tasks = [tasks_single; tasks_locationTasks];
                train = [train_single; train_locationTasks];
            end
...
%         [input_single, tasks_single, train_single] = createTaskPatterns(NPathways, NFeatures, samplesPerTask, sdScale, sameStimuliAcrossTasks, taskPair_dependent);
%         input_single(:, ((NPathways-1)*NFeatures+1):((NPathways)*NFeatures)) = 0;
%         [output, ~, MSE] = taskNet.runSet(input_single, tasks_single, train_single);
        ...
        if(~skipPRPAnalysis)
            ...
            [input, tasks, train, tasksIdxSgl] = createTaskPatterns(NPathways, NFeatures, [], sdScale, sameStimuliAcrossTasks, taskPair_dependent);
            [congruentIdx, incongruentIdx] = Part1_Sim3_getCongruentIdx(input, tasksIdxSgl, NPathways, taskA, taskB);
            input(:, ((NPathways-1)*NFeatures+1):((NPathways)*NFeatures)) = 0;
            ...
            for tau_idx = 1:length(tau_range)
                ...
                for SOA_idx = 1:length(SOA_range)
                    ...
                    [task1RT, task1Accuracy, task2RT, task2Accuracy, task1Iterations, task2Iterations, task1RT_all, task2RT_all, task1RT_distribution, task2RT_distribution] = Part1_Sim4_PRP_BCE_Analysis(SOA, taskNet, input, train, tasksIdxSgl, taskA, taskB, congruent, tauNet, tauTask, nStimuli, verbose);
                    task1RT_log(SOA_idx) = task1RT;
                    task1RT_all_log{SOA_idx} = task1RT_all;
                    task1RT_distribution_log{SOA_idx} = task1RT_distribution;
                    ...
                    task2RT_distribution_log{SOA_idx} = task2RT_distribution;
                    task2Accuracy_log(SOA_idx) = task2Accuracy;
                    task2Iterations_log(SOA_idx) = task2Iterations;
                end

                disp('PRP dependent congruent...');
                batch_log(numHiddenIdx, rep).funcDependence_con(tau_idx).task1RT_log = mean(task1RT_all_log{1}(congruentIdx));
                batch_log(numHiddenIdx, rep).funcDependence_con(tau_idx).task1RT_all_log = task1RT_all_log{1}(congruentIdx);
                batch_log(numHiddenIdx, rep).funcDependence_con(tau_idx).task1Accuracy_log = task1Accuracy_log;
                batch_log(numHiddenIdx, rep).funcDependence_con(tau_idx).task1Iterations_log = task1Iterations_log;
                batch_log(numHiddenIdx, rep).funcDependence_con(tau_idx).task2RT_log = mean(task2RT_all_log{1}(congruentIdx));
                batch_log(numHiddenIdx, rep).funcDependence_con(tau_idx).task2RT_all_log = task2RT_all_log{1}(congruentIdx);
                batch_log(numHiddenIdx, rep).funcDependence_con(tau_idx).task2Accuracy_log = task2Accuracy_log;
                batch_log(numHiddenIdx, rep).funcDependence_con(tau_idx).task2Iterations_log = task2Iterations_log;

                
                disp('PRP dependent incongruent...');
                batch_log(numHiddenIdx, rep).funcDependence_inc(tau_idx).task1RT_log = mean(task1RT_all_log{1}(incongruentIdx));
                batch_log(numHiddenIdx, rep).funcDependence_inc(tau_idx).task1RT_all_log = task1RT_all_log{1}(incongruentIdx);
                batch_log(numHiddenIdx, rep).funcDependence_inc(tau_idx).task1Accuracy_log = task1Accuracy_log;
                batch_log(numHiddenIdx, rep).funcDependence_inc(tau_idx).task1Iterations_log = task1Iterations_log;
                batch_log(numHiddenIdx, rep).funcDependence_inc(tau_idx).task2RT_log = mean(task2RT_all_log{1}(incongruentIdx));
                batch_log(numHiddenIdx, rep).funcDependence_inc(tau_idx).task2RT_all_log = task2RT_all_log{1}(incongruentIdx);
                batch_log(numHiddenIdx, rep).funcDependence_inc(tau_idx).task2Accuracy_log = task2Accuracy_log;
                batch_log(numHiddenIdx, rep).funcDependence_inc(tau_idx).task2Iterations_log = task2Iterations_log;
            ...
            %%
            tau_idx = 1;
            batch_log(numHiddenIdx, rep).funcDependence_inc(tau_idx).task1RT_log
            batch_log(numHiddenIdx, rep).funcDependence_inc(tau_idx).task2RT_log
            
%             batch_log(numHiddenIdx, rep).funcDependence_inc(tau_idx).task1Accuracy_log
%             batch_log(numHiddenIdx, rep).funcDependence_inc(tau_idx).task2Accuracy_log
            
            %%
            tau_idx = 1;
            batch_log(numHiddenIdx, rep).funcDependence_con(tau_idx).task1RT_log
            batch_log(numHiddenIdx, rep).funcDependence_con(tau_idx).task2RT_log
            
%             batch_log(numHiddenIdx, rep).funcDependence_con(tau_idx).task1Accuracy_log
%             batch_log(numHiddenIdx, rep).funcDependence_con(tau_idx).task2Accuracy_log
...
if(~trainMultitasking)
    save(['logfiles/Part1/PsychReview_Part1_Sim4_PRP_BCE_tau_' num2str(NPathways) 'P' num2str(NFeatures) 'F_' num2str(length(tasksToPerform)) 'tasks_' num2str(log_version) '_h' num2str(numHiddenUnits(1)) '_r' num2str(rep)]);
else
    save(['logfiles/Part1/PsychReview_Part1_Sim4_PRP_BCE_tau_multi_' num2str(NPathways) 'P' num2str(NFeatures) 'F_' num2str(length(tasksToPerform)) 'tasks_' num2str(log_version) '_h' num2str(numHiddenUnits(1)) '_r' num2str(rep)]);
end
